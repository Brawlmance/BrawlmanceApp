...
module.exports = app => {
  app.get('/v1/weapons', async function(req, res) {
    ...
    const aggregatedStats = await db.query(sqlQuery)
    const weaponsResult = []

    // Calculate averages and format the payload in memory
    for (const row of aggregatedStats) {
      if (!row.total_games) continue

      const games = parseFloat(row.total_games)
      const legends = parseFloat(row.total_legends)

      const winrate = parseFloat(row.total_wins) / games
      const damageWeaponOne = parseFloat(row.total_damage_w1) / games
      const damageWeaponTwo = parseFloat(row.total_damage_w2) / games
      const matchTime = parseFloat(row.total_matchtime) / games
      const timeheld1 = parseFloat(row.total_timeheld_w1) / games
      const timeheld2 = parseFloat(row.total_timeheld_w2) / games
