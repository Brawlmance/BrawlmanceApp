...
defer(['$'], function() {
      ...
      fetch('https://api.twitch.tv/helix/streams?game_id=460316&first=4', {
        ...
        .then(response => {
          for (key in response.data) {
            ...
            $('.streams').append(
              '<div><a target="_blank" rel="nofollow" href="https://www.twitch.tv/' +
                stream.user_name +
                '"><img src="' +
                stream.thumbnail_url.replace('{width}', '320').replace('{height}', '180') +
                '"/> <span class="name">' +
                stream.user_name +
                '</span><span class="viewers"><i class="fa fa-user"></i> ' +
                stream.viewer_count +
                '</span></a></div>'
            )
          }
      ...
      function e(e, n) {
        ...
        function w(e, t) {
          var r,
            n = e.elm
          return (
            t.selector && (t.hasFilter ? n.matchesSelector(t.selector) || (n = l) : (n = n.querySelector(t.selector))),
            t.hasAttr
        ...
        var D,
          ...
          O =
            e.length && (n === o || n.useFlex !== !1) && !L && -1 !== getComputedStyle(z, null).display.indexOf('flex')
        return (
    ...
    function n(o) {
      function t(n, r, i) {
        ...
        if ('undefined' != typeof document) {
          ...
          n || (c = {})
          for (
            var p = document.cookie ? document.cookie.split('; ') : [], s = /(%[0-9A-Z]{2})+/g, d = 0;
            d < p.length;
            d++
          ) {
            var f = p[d].split('='),
  ...
  var onorderfactorclick = function(e) {
    ...
    tinysort(grid.find('.card'), {
      selector: 'i[data-name="' + orderfactor + '"]',
      attr: 'data-value',
      order: neworder,
    })
